'''
    This module defines Jupyter Notebook "cell magic" to execute a SQLcl script in a remote shell.
    The SQLcl will be executed on a remote Unix after looging in and becoming oracle account via pbrun.
    The cell content is expected to be a valid SQLcl script.
    ...
    Once connected, a sqlcl is executed via pbrun as oracle run user, and cell content is fed to sqlcl via "here doc" construct.
    ...
    Usage examples: 
        ...
        In: %%pbrun_sqlcl user@host ssh_psw=password1 oracle_sid=ORCL1 oracle_conn="/ as sysdba"
    
            select sysdate from dual;
    
    Usage example as a regular python function:
    
        from pbrun_sqlcl import pbrun_sqlcl_fn
        shell_cmd="""
            select sysdate from dual;
        """
        out=pbrun_sqlcl_fn("user@host password ORACLE_SID ORACLE_CONN", sqlcl_cmds)
        
    The function does not stream back output, i.e. output does not appear until full script finishes.
...
@register_cell_magic
def pbrun_sqlcl(line, script):
    """
        This is a multiline cell magic function.
        It executes cell content via sqlcl on a remote host, after connecting to the remote host with ssh, 
        ...
        Usage example 2:
            ...
            %sqlcl conn=<user@host> psw=<password> oracle_sid=<sid> oracle_conn=<connection string>] [debug=True]
            select * from dual;
        ...
        Substitutions performed prior to sending script for remote execution: 
            ...
            - adding sqlcl here doc construct:
                    sqlcl -s -l {oracle_conn} <<-EOF
                            {script}
                    EOF
    ...
    # add sqlcl here doc
    script2 = _add_sqlcl_here_doc(script1, oracle_conn)
    ...
    # add html element with id="id_pbrun_sqlcl" for CSS to pick up
    #display( Javascript('element.setAttribute("id", "id_pbrun_sqlcl")') )
...
def _add_sqlcl_here_doc(script, oracle_conn):
    """
        add sqlcl here doc construct
        escape dollar sign
    """
    #script1= (f"newgrp oinstall\n"
    script1= (f"sqlcl -s -l {oracle_conn} <<-EOF"
              f"{script}"
              f"EOF")
    script2=re.sub(r"\$",
                   r"\\$",
